"""tests/quality shared fixtures：SQLite 会话 + 两用户 + task + API TestClient。"""
...
def db(tmp_path) -> DbSession:
    engine = create_engine(
        f"sqlite:///{tmp_path / 'quality.db'}", connect_args={"check_same_thread": False}
    )
...
def client(tmp_path) -> dict:
    """TestClient + sessionmaker，供 quality API 测试（同 tests/review 模式）。"""
    ...
    engine = create_engine(
        f"sqlite:///{tmp_path / 'quality_api.db'}", connect_args={"check_same_thread": False}
    )
    Base.metadata.create_all(engine)
    factory = sessionmaker(bind=engine, autoflush=False, expire_on_commit=False)
