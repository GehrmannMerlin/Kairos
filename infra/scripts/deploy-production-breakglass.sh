#!/usr/bin/env bash
# ⚠️ EMERGENCY ONLY — BREAK-GLASS DEPLOY PATH ⚠️
#
# This script transfers images via `docker save | ssh docker load`. It is NOT the
# standard Kairos deploy path (see agent-production-deployment-standards.md
# "Container Registry 与增量镜像发布"). Use ONLY when the OCI registry is
# confirmed unavailable AND the user explicitly approves a break-glass deploy.
# Record the reason + image digests + run a full smoke before/after.
